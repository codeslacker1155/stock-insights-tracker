      ...
      {loading ? (
        ...
        <div className="grid">
          ...
          <div className="box">
            ...
            <ul>
              {stockData.earningsChart.map((item, index) => (
                <li key={index}>
                  Date: {item.date}, Actual: {item.actual.fmt}, Estimate: {item.estimate.fmt}
                </li>
              ))}
            </ul>
          </div>
          <div className="box">
            ...
            <ul>
              {stockData.financialsChart.map((item, index) => (
                <li key={index}>
                  Date: {item.date}, Revenue: {item.revenue.fmt}, Earnings: {item.earnings.fmt}
                </li>
              ))}
            </ul>
          </div>
          <div className="box">
            ...
            <p>
              ...
              <strong>Total Cash: </strong>{stockData.totalCash}<br />
              <strong>Total Cash Per Share: </strong>{stockData.totalCashPerShare}<br />
              <strong>EBITDA: </strong>{stockData.ebitda}<br />
              ...
              <strong>Operating Cashflow: </strong>{stockData.operatingCashflow}<br />
              <strong>Earnings Growth: </strong>{stockData.earningsGrowth}<br />
              <strong>Revenue Growth: </strong>{stockData.revenueGrowth}<br />
              <strong>Gross Margins: </strong>{stockData.grossMargins}<br />
              <strong>EBITDA Margins: </strong>{stockData.ebitdaMargins}<br />
              <strong>Operating Margins: </strong>{stockData.operatingMargins}<br />
              <strong>Profit Margins: </strong>{stockData.profitMargins}<br />
              <strong>52 Week Change: </strong>{stockData.fiftyTwoWeekChange}<br />
              ...
              <strong>200 Day Moving Average: </strong>{stockData['200DayMovingAverage']}<br />
              ...
              <strong>Forward Annual Dividend Rate: </strong>{stockData.forwardAnnualDividendRate}
            </p>
          </div>
